...
const colors = {
  "Non-Indigenous":       "#CC3D48",
  "First Nations people": "#00546C",
};
...
const drawLineChart = (data) => {
  ...
  innerChart.append("path").attr("id", "line-main")
    ...
    .attr("fill", "none").attr("stroke", "#00546C").attr("stroke-width", 2);

    innerChart.selectAll(".circle-main")
    .data(initialData, d => d.year)
    .join("circle")
      ...
      .on("mouseover", function(event, d) {
        d3.select(this).transition().duration(100).attr("r", 7).attr("stroke", "#b5a48a").attr("stroke-width", 2);
        showTooltip(event, `
          <div class="chart-tooltip__title">${d.year}</div>
          <div class="chart-tooltip__value">${d3.format(",")(d.hospitalisations)}</div>
          <div class="chart-tooltip__sub">hospitalisations</div>
        `);
      })
      ...
      .on("mouseout", function() {
        const lineColor = currentStatus === "First Nations people" ? colors["First Nations people"]
          ...
          : "#00546C";
        d3.select(this).transition().duration(100).attr("r", 4).attr("stroke", "none").attr("fill", lineColor);
        hideTooltip();
      });
...
const attachDotTooltip = (selection, getColor) => {
  selection
    .on("mouseover", function(event, d) {
      d3.select(this).transition().duration(100).attr("r", 7).attr("stroke", "#b5a48a").attr("stroke-width", 2);
      showTooltip(event, `
        <div class="chart-tooltip__title">${d.group ? `${d.group === "Non-Indigenous" ? "Non-Indigenous" : "First Nations"} · ` : ""}${d.year}</div>
        <div class="chart-tooltip__value">${d3.format(",")(d.hospitalisations)}</div>
        <div class="chart-tooltip__sub">hospitalisations</div>
      `);
    })
    ...
    .on("mouseout", function(event, d) {
      d3.select(this).transition().duration(100).attr("r", 4).attr("stroke", "none").attr("fill", getColor(d));
      hideTooltip();
    });
};
...
const updateLineChart = (data) => {
  ...
  if (currentStatus === "concurrent") {
    ...
    const concurrentDots = chart.selectAll(".circle-concurrent")
    ...
    attachDotTooltip(chart.selectAll(".circle-concurrent"), d => colors[d.group]);
    ...
    const dots = chart.selectAll(".circle-main")
    ...
    attachDotTooltip(chart.selectAll(".circle-main"), () => lineColor);
  }
};
...
d3.csv("assets/data/populations_final.csv", d => ({
  ...
  function renderBarChart() {
    ...
    const fnFiltered = filtered.filter(d => d.status === "First Nations people");
    const niFiltered = filtered.filter(d => d.status === "Non-Indigenous");

    const fnRate = (d3.sum(fnFiltered, d => d.hospitalisations) / d3.sum(fnFiltered, d => d.population)) * 100000;
    const niRate = (d3.sum(niFiltered, d => d.hospitalisations) / d3.sum(niFiltered, d => d.population)) * 100000;

    const chartData = [
      {
        group: "First Nations",
        rate: Math.round(fnRate),
        hospitalisations: d3.sum(fnFiltered, d => d.hospitalisations),
        population: d3.sum(fnFiltered, d => d.population),
        color: "#00546C",
        hoverColor: "#003847"
      },
      {
        group: "Non-Indigenous",
        rate: Math.round(niRate),
        hospitalisations: d3.sum(niFiltered, d => d.hospitalisations),
        population: d3.sum(niFiltered, d => d.population),
        color: "#CC3D48",
        hoverColor: "#8C1F28"
      }
    ];
    ...
    g.selectAll(".bar")
      ...
      .on("mouseover", function(event, d) {
        d3.select(this).transition().duration(100).attr("fill", d.hoverColor);
        const isAllYears = selectedYear === "All";
        const isAllRemoteness = selectedRemoteness === "All";
        showTooltip(event, `
          <div class="chart-tooltip__title">${d.group}</div>
          <div class="chart-tooltip__value">${d.rate}</div>
          <div class="chart-tooltip__sub">per 100,000 population</div>
          <div class="chart-tooltip__sub">${d3.format(",")(d.hospitalisations)} hospitalisations</div>
          <div class="chart-tooltip__sub">pop. ${d3.format(",")(d.population)}</div>
          ${isAllYears ? `<div class="chart-tooltip__sub" style="margin-top:0.35rem; border-top: 1px solid rgba(255,255,255,0.15); padding-top:0.35rem;">Rate averaged across ${isAllYears ? "all years" : ""}${isAllYears && isAllRemoteness ? " & " : ""}${isAllRemoteness ? "all areas" : ""}</div>` : ""}
        `);
      })
      .on("mousemove", moveTooltip)
      .on("mouseout", function(event, d) {
        d3.select(this).transition().duration(100).attr("fill", d.color);
        hideTooltip();
      })
      .transition(t)
        .attr("x", d => xScale(d.group))
